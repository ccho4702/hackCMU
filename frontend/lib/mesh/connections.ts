...
export type MeshMode = "full" | "eyes" | "brows" | "mouth" | "contour";
...
export function connectionsFor(mode: MeshMode): number[][] {
  switch (mode) {
    case "eyes":
      return [
    ...
    case "contour":
      return [...connections.face_oval, ...connections.lips, ...connections.left_eye, ...connections.right_eye];
    default:
      return connections.tesselation;
  }
}

export function connectionsForRegion(region: "eyes" | "contour" | "mouthBrows"): number[][] {
  switch (region) {
    case "eyes":
