...
export async function beginCoaching(file: File, analysisId: string): Promise<string | undefined> {
  ...
  try {
    ...
    localStorage.setItem(`${key}.error`, error instanceof Error ? error.message : "Script and voice processing could not start.");
    return undefined;
  }
}
