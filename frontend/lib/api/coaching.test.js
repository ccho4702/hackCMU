...
test("retry preserves the originally selected language, accent and scenario and sends multipart data", async () => {
  ...
  expect(await beginCoaching(file,"analysis-1","en","british","interview")).toBeUndefined();
  expect(await beginCoaching(file,"analysis-1")).toBe("run-1");
  for (const [, request] of fetchMock.mock.calls) {
    expect(request.body.get("language")).toBe("en");
    expect(request.body.get("accent")).toBe("british");
    expect(request.body.get("script_style")).toBe("interview");
