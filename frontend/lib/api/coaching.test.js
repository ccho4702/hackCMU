...
test("retry preserves the originally selected language and scenario and sends multipart data", async () => {
  ...
  expect(await beginCoaching(file,"analysis-1","en","interview")).toBeUndefined();
  expect(await beginCoaching(file,"analysis-1")).toBe("run-1");
  for (const [, request] of fetchMock.mock.calls) {
    expect(request.body.get("language")).toBe("en");
    expect(request.body.has("accent")).toBe(false);
